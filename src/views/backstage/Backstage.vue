<template>
  <div class="Backstage">
    <div class="w">
      <div class="leftNav">
        <div class="box">
          <div class="img"><img v-lazy="user.src" src="" alt=""></div>
          <h2>{{user.username}}</h2>
          <p>会员ID:{{user.id}}</p>
        </div>
        <a-menu class="menu" mode="inline" :selected-keys="selectedKeys" :open-keys="openKeys" @openChange="onOpenChange">
          <template v-for="(item,index) in menu">
            <a-sub-menu :key="item.redirect">
              <span slot="title"><i :class="'icon iconfont ' + item.meta.icon"></i><span>{{item.meta.name}}</span></span>
              <template v-for="(item,sindex) in item.children">
                <a-menu-item :key="item.path" @click="itemClick(item.path)">
                  {{item.meta.name}}
                </a-menu-item>
              </template>
            </a-sub-menu>
      ...
      <div class="rightBox">
        <a-tabs @change="pathClick" v-model="activeKey">
          <a-tab-pane v-for="(item,index) in left[0].children" :key="index +1" :tab="item.meta.name">
          </a-tab-pane>
        </a-tabs>
        <keep-alive>
