<template>
  <div class="ManageSupplyDemand">
    <a-form-model ref="ruleForm" :model="form" :rules="rules" class="cForm">
      <a-row :gutter="14">
        <a-col :span="5">
          <a-form-model-item prop="type">
            <a-select v-model="form.type" placeholder="信息类型">
        ...
        <a-col :span="2">
          <a-form-model-item>
            <a-button class="nopadd orange" block type="primary" @click="onSubmit">搜索</a-button>
          </a-form-model-item>
        </a-col>
        <a-col :span="2">
          <a-form-model-item>
            <a-button class="nopadd default" block @click="resetForm">重置</a-button>
          </a-form-model-item>
        </a-col>
      </a-row>
    </a-form-model>
    ...
    <div class="cTable">
      <a-table :columns="columns" :data-source="data" :row-selection="{
        ...
        <template v-for="col in ['type', 'main', 'time']" :slot="col" slot-scope="text, record, index">
          <div :key="col">
      ...
      <a-button class="AllDelete" type="primary" @click="AllDelete">全部删除</a-button>
    </div>
  </div>
</template>
<script>
